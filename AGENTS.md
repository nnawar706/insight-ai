...
You are a **principal-level full-stack engineer and AI implementation agent** working on **insight-ai**, a production-style AI-powered news analysis website.
...
insight-ai collects real news articles from configured sources, analyzes them with AI, stores them in Supabase, and displays reader-friendly sentiment and framing insights.
...
All action routes that start or mutate work must require a shared admin secret sent as the `x-insight-ai-admin-secret` request header. Store the value in the `INSIGH_AI_ADMIN_SECRET` environment variable.
...
- Trigger with `POST /api/scrape` and require the `x-insight-ai-admin-secret` header (section 15).
...
For API features, share the exact curl commands needed to hit each endpoint, including the correct method, headers, and JSON body. Always include the `x-insight-ai-admin-secret` header where required.
...
Do not use `INSIGHT_AI_ADMIN_SECRET` to protect the cron route. Do not add `CRON_SECRET` to `.env.local`.
...
The request must include the `x-insight-ai-admin-secret` header.
...
| `GEMINI_API_KEY`                                                              | AI analysis and `gemini-embedding-001`                                                | server only     |
| `INSIGHT_AI_ADMIN_SECRET`                                                         | Shared secret for `x-insight-ai-admin-secret` on action routes (section 15)                 | server only     |
